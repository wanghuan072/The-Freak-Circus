  ...
  <div class="items-category-view">
    <div class="container">
      <div class="page-header">
        <h1 class="page-title">Ammo</h1>
        <p class="page-subtitle">
          Ammunition calibers and special rounds tailored for every firearm platform in VEIN survival game.
        </p>
      </div>
...
// 使用 computed 来响应语言变化
const roundsItems = computed(() => {
  const currentLang = getCurrentLocale() || locale.value || 'en'
  const targetType = TYPE_MAP[currentLang]?.rounds || TYPE_MAP.en.rounds
  const filtered = filterByType(targetType)
  if (import.meta.env.DEV) {
    console.log(`[ItemsAmmoView] roundsItems - lang: ${currentLang}, targetType: "${targetType}", count: ${filtered.length}, total items: ${itemsData.value?.length || 0}`)
  }
  return filtered
})
...
onMounted(async () => {
  ...
  if (import.meta.env.DEV) {
    console.log(`[ItemsAmmoView] onMounted - locale.value: ${locale.value}, getCurrentLocale(): ${currentLang}`)
  }
  await loadData('ammo', currentLang)
})

// 监听语言变化，重新加载数据
watch(() => locale.value, async (newLocale) => {
  if (import.meta.env.DEV) {
    console.log(`[ItemsAmmoView] Language changed to: ${newLocale}`)
  }
  await loadData('ammo', newLocale)
}, { immediate: false })
...
const onItemClick = (item) => {
  ...
  if (!id) return
  router.push(getLocalizedPath(`/vein-items/ammo/${id}`))
}
</script>
