    ...
    <div class="container">
      <div class="page-header">
        ...
        <p class="page-subtitle">
          Quest items, trade goods, collectibles, and everything else that doesn't fit the standard VEIN categories.
        </p>
      </div>

      <!-- Loading State -->
      <div v-if="loading" class="loading-state">
        <p>Loading misc items...</p>
      </div>

      <!-- Error State -->
      <div v-if="error" class="error-state">
        <p>Error loading misc items: {{ error }}</p>
      </div>

      <!-- Items Table -->
      <div v-if="!loading && !error && campingItems.length > 0" class="table-section">
        <div class="section-header">
          <h2 class="section-title">Camping</h2>
          <span class="section-count">{{ campingItems.length }} items</span>
        </div>
...
const campingItems = computed(() => {
  ...
  const targetType = TYPE_MAP[currentLang]?.camping || TYPE_MAP.en.camping
  const filtered = filterByType(targetType)
  if (import.meta.env.DEV) {
    console.log(`[ItemsMiscView] campingItems - lang: ${currentLang}, targetType: "${targetType}", count: ${filtered.length}, total items: ${itemsData.value?.length || 0}`)
  }
  return filtered
})
