    ...
    <div class="container">
      <div class="page-header">
        ...
        <p class="page-subtitle">
          Food, beverages, buffs, and temporary boosters that keep VEIN survivors energized during long sessions.
        </p>
      </div>
...
const TYPE_MAP = {
  ...
  de: {
    alcoholDrugs: 'alkohol-drogen',  // 对应 "Alkohol-Drogen"
  }
}
...
// 使用 computed 来响应语言变化
const alcoholDrugsItems = computed(() => {
  const currentLang = getCurrentLocale() || locale.value || 'en'
  const targetType = TYPE_MAP[currentLang]?.alcoholDrugs || TYPE_MAP.en.alcoholDrugs
  const filtered = filterByType(targetType)
  if (import.meta.env.DEV) {
    console.log(`[ItemsConsumablesView] alcoholDrugsItems - lang: ${currentLang}, targetType: "${targetType}", count: ${filtered.length}, total items: ${itemsData.value?.length || 0}`)
  }
  return filtered
})
...
onMounted(async () => {
  ...
  if (import.meta.env.DEV) {
    console.log(`[ItemsConsumablesView] onMounted - locale.value: ${locale.value}, getCurrentLocale(): ${currentLang}`)
  }
  await loadData('consumables', currentLang)
})

// 监听语言变化，重新加载数据
watch(() => locale.value, async (newLocale) => {
  if (import.meta.env.DEV) {
    console.log(`[ItemsConsumablesView] Language changed to: ${newLocale}`)
  }
  await loadData('consumables', newLocale)
}, { immediate: false })
...
const onItemClick = (item) => {
  ...
  if (!id) return
  router.push(getLocalizedPath(`/vein-items/consumables/${id}`))
}
</script>
